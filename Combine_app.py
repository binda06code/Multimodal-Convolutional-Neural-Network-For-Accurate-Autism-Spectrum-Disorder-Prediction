...
def multimodal_predict(image_paths, eye_tracking_file):
    try:
        # Image prediction
        image_class, image_prob = predictor(image_paths, image_model, classes, IMG_SIZE, 'ave')
        
        # Eye tracking prediction
        error, eye_tracking_results = eye_tracking_predict(eye_tracking_file)
        if error:
            return {'error': f"Eye tracking prediction error: {error}"}
        
        # Combine predictions
        eye_tracking_class = eye_tracking_results[0][1]
        final_result = 'Autistic' if (image_class == 'Autistic' or eye_tracking_class == 'Autistic') else 'Not Autistic'
        
        return {
            'image_class': image_class,
            'image_probability': image_prob,
            'eye_tracking_class': eye_tracking_class,
...
@app.route('/predict_multimodal')
def multimodal_pred():
    return render_template('base.html')

@app.route('/predict_multimodal', methods=['POST'])
def predict_multimodal():
    if 'image_files' not in request.files or 'eye_tracking_file' not in request.files:
        return render_template('base.html', prediction_text='No files uploaded.')

    image_files = request.files.getlist('image_files')
    eye_tracking_file = request.files['eye_tracking_file']
    ...
    prediction = multimodal_predict(image_paths, eye_tracking_file)
    if 'error' in prediction:
        return render_template('base.html', prediction_text=f"Error: {prediction['error']}")
    
    return render_template('base.html', prediction_text=f"Multimodal Prediction: {prediction['final_result']}")
